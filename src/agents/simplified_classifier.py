"""
Simplified Classifier Agent - Issue Classification and Initial Analysis
...
# Simple session state representation
...
class SimplifiedClassifierAgent:
    """Simplified classifier agent for issue classification."""
    ...
    def __init__(self):
        ...
        # Import vector search service lazily to avoid import issues
        self._vector_search_service = None
    
    @property
    def vector_search_service(self):
        """Lazy loading of vector search service."""
        if self._vector_search_service is None:
            from src.core.simple_vector_search import SimplifiedVectorSearchService
            self._vector_search_service = SimplifiedVectorSearchService()
        return self._vector_search_service
    
    async def execute(self, session_state: SimpleSessionState, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            search_results = await self.vector_search_service.search_similar_issues(
                user_input,
            ...
            classification_result = await self.vector_search_service.classify_issue(user_input)
    ...
    async def _generate_classification_candidates(self,
        ...
        for i, result in enumerate(search_results):
            ...
            candidate = {
                ...
                'diagnostic_questions': result.diagnostic_questions,
                'tools': result.tools,
                'match_reasons': []
            }
    ...
    async def _generate_diagnostic_questions(self,
        ...
        """Generate context-aware diagnostic questions using advanced question generator."""
        
        try:
            # Import here to avoid circular dependencies
            from src.core.diagnostic_questions import get_diagnostic_generator, QuestionStrategy
            
            # Get the diagnostic question generator
            generator = get_diagnostic_generator()
            
            # Convert classification result to expected format
            classification_dict = {
                "suggested_category": classification_result.suggested_category,
                "confidence": classification_result.confidence,
                "matched_title": classification_result.matched_title,
                "classified": classification_result.classified
            }
            
            # Prepare entities for question generator (convert to expected format)
            enhanced_entities = {
                "menu_item": [entities["dish_name"]] if entities.get("dish_name") else [],
                "product": [entities["dish_name"]] if entities.get("dish_name") else [],
                "warehouse": [entities["warehouse"]] if entities.get("warehouse") else [],
                "location": [entities["warehouse"]] if entities.get("warehouse") else [],
                "time": [entities["date_period"]] if entities.get("date_period") else [],
                "time_indicators": [entities["date_period"]] if entities.get("date_period") else [],
                "system": entities.get("system_components", []),
                "error": ["true"] if entities.get("error_detected") else [],
                "issue_type": [patterns.get("primary_issue_type")] if patterns.get("primary_issue_type") else [],
                "keywords": entities.get("keywords", [])
            }
            
            # Add confidence scores as entities for question generation
            for pattern_type, confidence in patterns.get("confidence_scores", {}).items():
                if confidence > 0.5:
                    enhanced_entities[f"_{pattern_type}"] = [str(confidence)]
            
            # Generate questions using adaptive strategy
            question_result = await generator.generate_questions(
                classification_result=classification_dict,
                entities=enhanced_entities,
                detected_patterns=patterns,
                session_state={"session_id": "classifier_session"},
                strategy=QuestionStrategy.ADAPTIVE,
                max_questions=4
            )
            
            if question_result.success:
                # Return Vietnamese questions for user interaction
                vietnamese_questions = [q.vietnamese_question for q in question_result.questions]
                self.logger.info(f"Generated {len(vietnamese_questions)} diagnostic questions")
                return vietnamese_questions
            else:
                self.logger.warning(f"Question generation failed: {question_result.error}")
                # Fall back to basic questions
                
        except Exception as e:
            self.logger.error(f"Advanced question generation failed: {str(e)}")
        
        # Fallback to basic questions if advanced generation fails
        questions = []
    ...
    async def validate_input(self, session_state: SimpleSessionState, **kwargs) -> bool:
    ...
    async def handle_error(self, error: Exception, session_state: SimpleSessionState) -> Dict[str, Any]:
        ...
        from src.core.simple_vector_search import ClassificationResult
...
simplified_classifier_agent = SimplifiedClassifierAgent()


async def get_simplified_classifier_agent() -> SimplifiedClassifierAgent:
    """Get the global simplified classifier agent instance."""
    return simplified_classifier_agent