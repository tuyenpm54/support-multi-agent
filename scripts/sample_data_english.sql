-- Sample Data for Hierarchical Issue Resolution Database (English Version)
...
('Formula and Pricing Issues',
 ...
 '["formula", "pricing", "calculation", "reports", "display"]'),
...
('Database Connection Issues',
 ...
 '["database", "connection", "query", "performance", "optimization"]'),
...
('System Performance Issues', 
 ...
 '{"approach": "sequential", "user_confirmation_required": true}',
 '["performance", "speed", "optimization", "monitoring", "efficiency"]'),
...
('Authentication and Authorization Issues',
 ...
 '{"approach": "sequential", "user_confirmation_required": true}',
 '["authentication", "login", "authorization", "session", "security"]');

-- Get the general issue IDs for parent-child relationships
...
SELECT 
    ...
    '["fried rice", "formula", "price", "display", "reports"]'
...
SELECT 
    ...
    '["beef noodle", "price error", "formula", "calculation", "incorrect"]'
FROM issues gi WHERE gi.title = 'Formula and Pricing Issues' AND gi.issue_type = 'general' LIMIT 1;
...
SELECT 
    ...
    '["bubble tea", "divide by zero", "formula validation", "calculation", "error"]'
FROM issues gi WHERE gi.title = 'Formula and Pricing Issues' AND gi.issue_type = 'general' LIMIT 1;
...
SELECT 
    ...
    '["database", "connection timeout", "report queries", "performance", "index"]'
...
SELECT 
    ...
    '["slow query", "monthly reports", "performance", "optimization", "index"]'
FROM issues gi WHERE gi.title = 'Database Connection Issues' AND gi.issue_type = 'general' LIMIT 1;
...
SELECT 
    ...
    '["login", "JWT", "session", "token expiration", "authentication"]'
...
SELECT 
    ...
    COUNT(*) as count,
    STRING_AGG(SUBSTRING(title FROM 1 FOR 30), ', ' ORDER BY title) as items
